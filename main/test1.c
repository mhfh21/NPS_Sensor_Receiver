...
#define BUFFER_SIZE 50 // اندازه بافر برای دریافت دسته‌ای داده‌ها
...
void receiver(void *pvParameters)
{
    ...
    uint8_t channels[] = {90, 91}; // کانال‌ها برای هر دو فرستنده
    ...
    uint8_t buf[32];
    uint8_t batch_buffer[BUFFER_SIZE][32];
    int buffer_index = 0;

    while(1) {
        current_channel = (current_channel + 1) % 2; // تغییر کانال
        Nrf24_config(&dev, channels[current_channel], payload);
        ESP_LOGI(TAG, "Switched to channel %d", channels[current_channel]);

        while(Nrf24_dataReady(&dev)) {
            Nrf24_getData(&dev, buf);
            ESP_LOGI(TAG, "Received data from channel %d: %s", channels[current_channel], buf);

            // ذخیره داده‌ها در بافر
            strncpy((char*)batch_buffer[buffer_index], (char*)buf, 32);
            buffer_index++;

            // اگر بافر پر شد، داده‌ها را به سرور بفرست
            if (buffer_index == BUFFER_SIZE) {
                for (int i = 0; i < BUFFER_SIZE; i++) {
                    send_data_to_server(batch_buffer[i]);
                }
                buffer_index = 0; // بازنشانی ایندکس بافر
            }
        }
        
        vTaskDelay(100 / portTICK_PERIOD_MS); // تأخیر بین تغییر کانال
    }
}
...
void send_data_to_server(uint8_t *data) {
    ...
    ESP_LOGI(TAG, "Connected to server, sending data...");
